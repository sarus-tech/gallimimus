...
from gallimimus.codec import CategoricalCodec, ListCodec, StructCodec, SharedCodec, LoraCodec
...
dataset = [
    (jnp.array(i % max_cat), jnp.array(i**2 % max_cat)) for i in range(N)
]

dataset = [
    jnp.array(i**2 % max_cat) for i in range(N)
]
...
shared_cat_codec = SharedCodec(
    embed_dim = embed_dim,
    shared_module_name = "cat_codec"
)
