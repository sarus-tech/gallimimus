...
class TextCodec(Codec):
    ...
    def encode(
        ...
        pos_emb = gpt2_model.wpe.apply(
            variables = {"params": gpt2_params["wpe"]}, 
            inputs = position_ids.astype("i4")
        )
        
        tok_emb = gpt2_model.wte.apply(
            variables = {"params": gpt2_params["wte"]}, 
    ...
    def decode(
        ...
        hidden_states = gpt2_model.ln_f.apply(
            {"params": gpt2_params["ln_f"]},
            hidden_states)
        
        predictions = gpt2_model.wte.apply(
            {"params": gpt2_params["wte"]},
            hidden_states, method="attend")
    ...
    def sample(
        ...
        def _sample_gpt2(self, carry: SampleState, sequences: jnp.ndarray):
            ...
            hidden_states = gpt2_model.h.apply(
                {"params": gpt2_params["h"]},
                embeddings[None, :], attention_mask)[0]
            ...
            pos_emb = gpt2_model.wpe.apply(
                {"params": gpt2_params["wpe"]},
                position_id_next_token
            )
            tok_emb = gpt2_model.wte.apply(
                {"params": gpt2_params["wte"]},next_token)
        ...
        start_embeddings = jnp.concatenate(
            [bias_embeddings, start_embeddings], axis=-2
        )
        pad_token_id = jnp.array(self.bos_token_id + 1, dtype=jnp.int32)
        init_sequence = jnp.full(
            (self.max_length,), pad_token_id, dtype=jnp.int32
        )
        ...
        hidden_states = gpt2_model.h.apply(
                {"params": gpt2_params["h"]},
            carry.running_embeddings[None, :], carry.attention_mask
        )[0]
        hidden_states = gpt2_model.ln_f.apply(
            {"params": gpt2_params["ln_f"]},hidden_states)[0]
    ...
    def loss(
        ...
        loss = jnp.sum(loss, axis=-1) / jnp.sum(
