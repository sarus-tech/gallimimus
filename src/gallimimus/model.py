...
class MetaLearner:
    ...
    generates is defined by the provided ``Codec``. The type of data is described as a
    graph using the nodes in ``model_dict``.

    :param codec_in: The codec corresponding to the data type generated by the model.
    :param model_dict: A collection of codecs and other models that can be shared in the
        type-tree.
    :param params_dict: A collection of parameters for the shared codecs. For each model
        ...
        - If the parameters are not provided, they need to be initialized. This is
            always possible for a codec. However, a model in ``model_dict`` requires
            an initialization function in ``init_fn_dict`` when no pretrained
            parameters are provided.
    :param init_fn_dict: A dictionary of parameter initialization function for each
        model (other than codecs).
    """
